 ...
 * Note: Telegram bot configuration has been removed.
 */
class ProfileView {
  renderView() {
    ...
    if (currentUser) {
      ...
      if (displayStatus) {
        if (currentUser.isOfficial) {
          displayStatus.innerHTML = '<span class="badge badge-official">✔ Học viên chính thức</span>';
        } else {
          displayStatus.innerHTML = '<span class="badge badge-guest">Học viên thường</span>';
        }
      }
    } else {
  ...
  saveProfile(event) {
    ...
    try {
      window.authService.updateProfile({ fullname, email, phone });
