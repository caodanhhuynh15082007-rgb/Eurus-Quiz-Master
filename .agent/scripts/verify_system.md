...
1. **Inspect Project Test Runner**: Read `ARCHITECTURE.md` or `active_context.md` to identify the native verification command (e.g. `npm test`, `cargo test`, `pytest`, `go test`).
2. **Execute Local Validation**: Run the detected verification command.
3. **Handle Results**:
   - If Pass: Update `active_context.md` checkpoint.
